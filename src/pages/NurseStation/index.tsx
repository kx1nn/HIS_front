...
const NurseStation: React.FC = () => {
  ...
  const initialDoctorId = initialActiveDoctors.length > 0 ? String(initialActiveDoctors[0].id) : '';
            ...
            <div className="space-y-3">
                ...
                {departments.map(dept => (
                  <button 
                    ...
                    onClick={() => {
                      ...
                      const newActiveDoctors = doctors.filter(d => d.deptId === newDeptId && d.isWorking);
                      const newDoctorId = newActiveDoctors.length > 0 ? String(newActiveDoctors[0].id) : '';
                      setFormData({...formData, deptId: newDeptId, doctorId: newDoctorId});
                    }}
              ...
              <div className="grid grid-cols-1 gap-2 max-h-32 overflow-y-auto">
                {activeDoctors.length > 0 ? (
                  ...
                  <div 
                    key={doc.id}
                    onClick={() => setFormData({...formData, doctorId: String(doc.id)})}
                    className={`p-3 border rounded-xl cursor-pointer flex justify-between items-center transition-all ${String(formData.doctorId) === String(doc.id) ? 'bg-teal-50 border-teal-500 shadow-sm' : 'bg-white border-slate-200 hover:border-teal-300'}`}
                  >
